...
const renderForm = () =>
  renderWithProviders(
    <Routes>
      <Route path="/dashboard/edit-donation/:id" element={<EditDonationRequest />} />
    </Routes>,
    { user: donorUser, route: '/dashboard/edit-donation/req-1' },
  );

describe('EditDonationRequest', () => {
  ...
  it('saves the edited request', async () => {
    getDonationRequest.mockResolvedValue(request);
    ...
    expect(updateDonationRequest).toHaveBeenCalledWith(
      'req-1',
      expect.objectContaining({ recipient_name: 'Mona Ali', blood_group: 'A+' }),
    );
  });
});
