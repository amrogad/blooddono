...
const HeroLiveCard = ({ req, onDonate, onDetails }) => {
  ...
  return (
    <div className="relative">
      <div className="absolute -end-3 top-10 bottom-2 start-8 rotate-2 rounded-3xl border border-line bg-card/70" />
      <div className="absolute end-1 top-7 bottom-4 start-4 -rotate-1 rounded-3xl border border-line bg-card/80" />
      <div className="relative rounded-3xl border border-line bg-card p-5 shadow-[0_32px_64px_-28px_rgba(120,20,40,0.35)]">
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <span dir="auto" className="truncate text-[16.5px] font-semibold text-ink">
                {req.recipient_name}
              </span>
              <UrgencyPill level={level} />
            </div>
            <div dir="auto" className="mt-0.5 truncate text-[13.5px] text-muted">
              {[localizeCity(req.recipient_city), localizeGov(req.recipient_governorate)]
                .filter(Boolean)
                .join(', ')}
            </div>
          </div>
        ...
        <div className="flex gap-2.5">
          ...
          >
            <LuDroplet className="h-4 w-4" strokeWidth={2.2} />
            {t('card.canDonate')}
          </button>
          <button
            onClick={onDetails}
      ...
      <div className="mx-auto grid max-w-[1180px] items-center gap-14 px-6 py-16 lg:grid-cols-2 lg:py-20">
        <div>
          <span className="inline-flex items-center gap-2 rounded-full border border-crimson/20 bg-crimson-tint px-3.5 py-1.5 text-[12.5px] font-semibold text-crimson">
            ...
            {openCount > 0
              ? t('home.hero.badgeOpen', { count: openCount })
              : t('home.hero.badgeEmpty')}
          </span>
          ...
          <p className="mt-5 max-w-md text-lg leading-relaxed text-body">{t('home.hero.subtitle')}</p>
